    ...
    methods: {
        ...
        checkBetMoney () {
            ...
            if (Number(this.areaMsg.pickMoney) > parseFloat(this.max_limit)) {
                ...
                this.$error(_(`Bet amount is between ${this.min_limit} and ${this.max_limit} ${formateCoinType(this.currBalance.cointype)}`))
                return false
            }
        ...
        js_beting_low () {
            ...
            if (currpickMoney <= parseFloat(this.min_limit)) {
                this.areaMsg.pickMoney = parseFloat(this.min_limit)
                this.$error(_(`The minimum bet is ${this.min_limit} ${formateCoinType(this.currBalance.cointype)}`))
            } else {
        ...
        lineNumClick ($event) {
            ...
            if ($event.target.tagName === 'LI') {
                ...
                if ($event.target.className === 'on') {
                    /* 清楚数据 */
                    currpickNum.indexOf(dataFlag) !== -1 ? currpickNum.splice(currpickNum.indexOf(dataFlag), 1) : console.warn('lineNumClick error')
                } else {
