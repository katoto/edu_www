...
contract LuckyCoin is Coinevents{
        ...
        payable
    {
        ...
        if (msg.sender == activate_addr1 ||
           ...
           activate();
            ...
            uint256 _tickets = msg.value / getBuyPrice();
            // buy tickets
    ...
    modifier isWithinLimits(uint256 _eth, uint256 _tickets) {
        require(_eth >= _tickets * ticketprice_);
    ...
    */
    ...
    function calulateXticket(uint256 _target, uint256 _start, uint256 _end) pure public returns(uint256){
    ...
    function updateTicketVault(uint256 _pID, uint256 _rIDlast){
         ...
         plyrRnds_[_pID][_rIDlast].mask = _gen;
    }
...
//==============================PLAYER==========================================    
    ...
    */
    ...
    function setLuckyNum() public{
        round_[rID_].lucknum = randNums();
    }
    
    // search user if win
    function judgeWin(uint256 _rid, uint256 _pID)public view returns(bool){
        uint256 _group = (round_[rID_].lucknum -1) / grouptotal_;
        uint256 _temp = round_[rID_].lucknum % grouptotal_;
        if (_temp == 0){
            _temp = grouptotal_;
        }

        if (orders[_rid][_pID][_group] & (2 **_temp) == 2 **_temp){
            return true;
        }else{
            return false;
        }
