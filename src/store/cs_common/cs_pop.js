...
const actions = {
  ...
  async userLogin({ commit, dispatch }, pageData) {
    try {
      let InfoData = null
      if (pageData) {
        InfoData = await ajax.get(
          `/user/login?email=${pageData.email}&password=${md5(md5(pageData.password))}&verify_code=${
            pageData.verify_code
          }`
        )
      }
  ...
  async resetPasswordFn({ state, commit, dispatch }, pageData) {
    try {
      ...
      if (pageData) {
        InfoData = await ajax.get(
          `/user/reset/password?email=${pageData.email}&sign=${pageData.sign}&password=${md5(md5(pageData.password))}`
        )
      }
      return InfoData
    } catch (e) {
